...
async function CrearBaseSiNoExiste() {
    ...
    let existeTipos = false;
    let sql =
        "SELECT count(*) as contar FROM sqlite_schema WHERE type = 'table' and name= ' TiposCongreso'";
    let res = await db.get(sql, []);
    if (res.contar > 0) existeTipos = true;
    if (!existeTipos) {
        await db.run(`
        ...
        `);
        console.log( "se creo tipos")
        await db.run(`INSERT INTO TiposCongreso (Nombre, Descripción, Participantes) VALUES 
        ...
        `);console.log( "se inserto tipos")
        }


        let existeOradores = false;
        sql =
            "SELECT count(*) as contar FROM sqlite_schema WHERE type = 'table' and name= 'Oradores'";
        res = await db.get(sql, []);
        if (res.contar > 0)existeOradores = true;
        if (!existeOradores) {
        await db.run(`
        ...
        let existePatrocinadores = false;
        sql =
            "SELECT count(*) as contar FROM sqlite_schema WHERE type = 'table' and name= 'Patrocinadores'";
        res = await db.get(sql, []);
        if (res.contar > 0)existePatrocinadores = true;
        if (!existePatrocinadores) {
        await db.run(`
    ...
    let existeParticipantes = false;
    sql =
        "SELECT count(*) as contar FROM sqlite_schema WHERE type = 'table' and name= 'Participantes'";
    res = await db.get(sql, []);
    if (res.contar > 0)existeParticipantes = true;
    if (!existeParticipantes) {
        await db.run(`
    ...
    let existeEvaluaciones = false;
    sql =
        "SELECT count(*) as contar FROM sqlite_schema WHERE type = 'table' and name= 'Evaluaciones'";
    res = await db.get(sql, []);
    if (res.contar > 0)existeEvaluaciones = true;
    if (!existeEvaluaciones) {
        await db.run(`
    ...
    let existeInscripciones = false;
    sql =
        "SELECT count(*) as contar FROM sqlite_schema WHERE type = 'table' and name= 'Inscripciones'";
    res = await db.get(sql, []);
    if (res.contar > 0)existeInscripciones = true;
    if (!existeInscripciones) {
        await db.run(`
